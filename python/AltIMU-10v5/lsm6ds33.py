...
class LSM6DS33(I2C):
    ...
    def getAccelerometerRaw(self):
        ...
        # Check XLDA: 0bXXXXXXX1
        LSM_XLDA = 0x01
        while (self._read_register(0x6b, self.LSM_STATUS_REG) & LSM_XLDA) == 0:
            continue
    ...
    def getGyroscopeRaw(self):
        ...
        # Check GDA: 0bXXXXXX1X
        LSM_GDA = 0x02
        while (self._read_register(0x6b, self.LSM_STATUS_REG) & LSM_GDA) == 0:
            # logging.error('#IMU: Gyroscope data not available')
            continue

        # Read sensor data
        return self._get_sensor_raw_lohi3(LSM6DS33_ADDR, self.lsmGyroRegisters)
    ...
    def getLSMTemperatureRaw(self):
        ...
        # Check TDA: 0bXXXXX1XX
        LSM_TDA = 0x04
        while (self._read_register(0x6b, self.LSM_STATUS_REG) & LSM_TDA) == 0:
            # logging.error('#IMU: Thermometer data not available')
            continue

        # Read sensor data
        return self._get_sensor_raw_lohi3(LSM6DS33_ADDR, self.lsmTempRegisters)
